...
class2 = pd.read_csv('dataset.csv')
## change the labels 
for i in range(len(class2)):
  if class2.Class[i] == 'sit and stand':
    class2.Class[i] = 'no-fall'
  elif class2.Class[i] == 'stand and bend':
    class2.Class[i] = 'no-fall'
  elif class2.Class[i] == 'walking':
    class2.Class[i] = 'no-fall'
  elif class2.Class[i] == 'walk up and down':
    class2.Class[i] = 'no-fall'
  elif class2.Class[i] == 'jogging':
    class2.Class[i] = 'no-fall'
  elif class2.Class[i] == 'lying down':
    class2.Class[i] = 'no-fall'
  elif class2.Class[i] == 'sit and collapsed when standing':
    class2.Class[i] = 'no-fall'
  elif class2.Class[i] == 'jump':
    class2.Class[i] = 'no-fall'
  elif class2.Class[i] == 'stand and get in car':
    class2.Class[i] = 'no-fall'
  elif class2.Class[i] == 'stumble':
    class2.Class[i] = 'no-fall'
  elif class2.Class[i] == 'fall forward':
    class2.Class[i] = 'fall'
  elif class2.Class[i] == 'fall backward':
    class2.Class[i] = 'fall'
  elif class2.Class[i] == 'lateral fall':
    class2.Class[i] = 'fall'

## find NaN rows
class2[class2.isnull().any(axis = 1)]
## fill NaN
class2.stdX = class2.stdX.fillna(0)
class2.stdY = class2.stdY.fillna(0)
class2.stdZ = class2.stdZ.fillna(0)

print("Number of Cases in Each Class - 2 Classes")
print("-----------------------------------------")
class2.Class.value_counts()
...
X = np.array(principal_4_Df.iloc[:,:-1])
y = np.array(principal_4_Df.iloc[:,-1])
model = svm.SVC(kernel='rbf', gamma=1, C=1, decision_function_shape='ovo', probability=True).fit(X, y)
kf = KFold(n_splits=10, shuffle=True, random_state = 0)
...
for train_index, test_index in kf.split(X):
  ...
  score = cross_val_score(model, X_test, y_test, cv=kf).mean() 
  overall.append(score)
  trial+=1
  print("RBF SVM  4 dim PCA")
...
model = svm.SVC(kernel='rbf', gamma=1, C=1, decision_function_shape='ovo', probability=True).fit(X, y)
kf = KFold(n_splits=10, shuffle=True, random_state = 0)
rus = RandomUnderSampler(sampling_strategy=0.8, random_state=0)
...
for train_index, test_index in kf.split(X):
  ...
  overall.append(score)
  trial+=1
  print("RBF SVM  4 dim PCA  Best Classifier under-sample by 0.8")
  print("Trial {}" .format(trial)) 
  print("Accuracy: {:.2%}".format(score))
  print("Sensitivity: ", recall_score(y_test, y_pred, average=None))
  print("Specificity: ", Specificity())
...
########## one sided selection (sampling_strategy = 'no-fall')##########
X = np.array(principal_4_Df.iloc[:,:-1])
y = np.array(principal_4_Df.iloc[:,-1])
model = svm.SVC(kernel='rbf', gamma=1, C=1, decision_function_shape='ovo', probability=True).fit(X, y)
kf = KFold(n_splits=10, shuffle=True, random_state = 0)
rus = OneSidedSelection(n_neighbors=8, sampling_strategy=['no-fall'])
pipeline = make_pipeline(rus, model)
X_res , y_res = rus.fit_resample(X, y)
...
for train_index, test_index in kf.split(X):
  ...
  overall.append(score)
  trial+=1
  print("RBF SVM  4 dim PCA  Best Classifier under-sample by OSS")
  print("Resampled Class: no-fall")
  print("Trial {}" .format(trial)) 
  print("Accuracy: {:.2%}".format(score))
  print("Sensitivity: ", recall_score(y_test, y_pred, average=None))
  print("Specificity: ", Specificity())
